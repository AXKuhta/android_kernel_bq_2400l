			...
			struct sprd_nand_timing *timing, uint32_t clk_hz)
{
	...
	/* get rwh value */
	temp_val = timing->rwh_ns * clk_mhz / 1000;
	if (((timing->ace_ns * clk_mhz) % 1000)  != 0)
		temp_val++;
	...
	/* get rwl value */
	temp_val = timing->rwl_ns * clk_mhz / 1000;
	if (((timing->ace_ns * clk_mhz) % 1000)  != 0)
		temp_val++;
		...
		uint32_t num, struct mtd_ecc_stats *ecc_sts, uint32_t mode)
{
	...
	for (i = 0; i <= ecc_banknum; i++) {
		for (n = 0; n < min((num - sector_num*i), sector_num); n++) {
			...
			case 0x03:
				/* fail */
				ret = sprd_nand_check_ff(host, i, mode);
				if (ret == -EBADMSG) {
					ecc_sts->failed++;
				} else {
						...
						struct sprd_nand_host *host)
{
	...
	pr_info("nand:sprd nand irq = %d ,freq = %d\n",
			(uint32_t) host->irq, host->frequence);
...
static int sprd_and_drv_probe(struct platform_device *pdev)
{
	struct sprd_nand_host *host;
	struct sprd_nfc_base *nfc_base;
	uint32_t i;
