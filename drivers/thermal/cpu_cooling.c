				    ...
				    unsigned long event, void *data)
{
	...
	list_for_each_entry(cpufreq_dev, &cpufreq_dev_list, node) {
		...
		clipped_freq = min((unsigned int)clipped_freq,
			cpufreq_dev->curr_max_freq);
			...
			u32 target_power, unsigned int target_freq)
{
	...
	estimated_power = 0;
	num_our_online_cpus = num_our_online_cpus + 1;
	per_cpu_load = cpufreq_device->last_load / num_our_online_cpus;
	per_cpu_load = per_cpu_load ?: 1;
			       ...
			       unsigned long state, u32 *power)
{
	...
	if (freq > cpufreq_device->curr_max_freq)
		freq = cpufreq_device->curr_max_freq;
	if (!freq)
		return -EINVAL;

	dynamic_power = cpu_freq_to_power(cpufreq_device, freq) * num_cpus;
	dynamic_power += l2_freq_to_power(cpufreq_device, freq);
	ret = get_static_power(cpufreq_device, tz, freq, &static_power);
	if (ret)
		return ret;
					...
					struct thermal_zone_device *tz)
{
	...
	if (allow_max_freq) {
		...
		pr_info("cpu%u forced max_freq to %u from %u\n", cpu,
