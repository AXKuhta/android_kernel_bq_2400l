...
struct regulator *vddsim2_supply;
							...
							unsigned long arg)
{
	...
	case SEC_NFC_ENABLE_SIMVDD2:
		dev_dbg(info->dev, "%s: SEC_NFC_ENABLE_SIMVDD2\n", __func__);
		if (IS_ERR(vddsim2_supply)) {
			ret = PTR_ERR(vddsim2_supply);
			dev_err(info->dev, "failed to get regulator vddsim2\n");
		} else {
			ret = regulator_enable(vddsim2_supply);
			if (ret < 0) {
				dev_err(info->dev, "failed to enable vddsim2 regulator\n");
			} else {
				ret = regulator_set_voltage(vddsim2_supply, 1800000, 1800000);
				if (ret < 0)
					dev_err(info->dev, "failed to set voltage vddsim2\n");
			}
		}
		break;

	case SEC_NFC_DISABLE_SIMVDD2:
		dev_dbg(info->dev, "%s: SEC_NFC_DISABLE_SIMVDD2\n", __func__);
		if (IS_ERR(vddsim2_supply)) {
			ret = PTR_ERR(vddsim2_supply);
			dev_err(info->dev, "failed to get regulator vddsim2\n");
		} else {
			regulator_disable(vddsim2_supply);
		}
		break;
	...
	struct sec_nfc_platform_data *pdata)
{
	...
	vddsim2_supply = devm_regulator_get_optional(dev, "vddsim2");
	return 0;
}
