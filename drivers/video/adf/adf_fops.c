...
static int adf_buffer_simple_import(struct adf_device *dev,
		struct adf_buffer_config  *cfg, struct adf_buffer *buf)
{
	size_t i;
	int ret = 0;
	...
	if (cfg->n_planes > ADF_MAX_PLANES) {
		dev_err(&dev->base.dev, "invalid plane count %u\n",
				cfg->n_planes);
		return -EINVAL;
	}
	...
	buf->w = cfg->w;
	buf->h = cfg->h;
	buf->format = cfg->format;
	for (i = 0; i < cfg->n_planes; i++) {
		buf->dma_bufs[i] = dma_buf_get(cfg->fd[i]);
		if (IS_ERR(buf->dma_bufs[i])) {
			ret = PTR_ERR(buf->dma_bufs[i]);
			dev_err(&dev->base.dev, "importing dma_buf fd %d failed: %d\n",
					cfg->fd[i], ret);
		...
		buf->offset[i] = cfg->offset[i];
		buf->pitch[i] = cfg->pitch[i];
	}
	buf->n_planes = cfg->n_planes;

	if (cfg->acquire_fence >= 0) {
		buf->acquire_fence = sync_fence_fdget(cfg->acquire_fence);
		if (!buf->acquire_fence) {
			dev_err(&dev->base.dev, "getting fence fd %d failed\n",
					cfg->acquire_fence);
			ret = -EINVAL;
		...
		struct adf_post_config __user *arg)
{
	...
	data.complete_fence = complete_fence_fd;
	if (copy_to_user(arg, &data, sizeof(data))) {
		ret = -EFAULT;
		goto err_import;
	}

	complete_fence = adf_device_post_nocopy(dev, intfs,
				data.n_interfaces, bufs,
				data.n_bufs, custom_data,
				data.custom_data_size);
		...
		struct adf_simple_post_config __user *arg)
{
	...
	ret = adf_buffer_simple_import(dev, &data.buf, &buf);
	if (ret < 0)
		goto err_import;
