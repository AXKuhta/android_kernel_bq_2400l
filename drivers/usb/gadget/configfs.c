...
static void android_work(struct work_struct *data)
{
	...
	if (status[0]) {
		if (gi->delay_event == true) {
			spin_lock_irqsave(&gi->slock, flags);
			gi->delay_event = false;
			spin_unlock_irqrestore(&gi->slock, flags);
			kobject_uevent_env(&android_device->kobj,
					KOBJ_CHANGE, disconnected);
			pr_info("%s: sent delay uevent %s\n",
	...
	if (!uevent_sent) {
		pr_info("%s: did not send uevent (%d %d %d %p)\n", __func__,
			gi->connected, gi->sw_connected,
			gi->delay_event, cdev->config);
	}
}
#endif
...
static void android_disconnect(struct usb_gadget *gadget)
{
	...
	*/
	...
	spin_lock_irqsave(&gi->slock, flags);
	gi->connected = 0;
	if (gadget->state == USB_STATE_REMOVED)
		gi->delay_event = false;
	else
		gi->delay_event = true;
	schedule_work(&gi->work);
