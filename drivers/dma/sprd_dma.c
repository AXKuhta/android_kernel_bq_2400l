			 ...
			 u32 node_size)
{
	...
	pr_err(DMA_TAG "Linklist:alloc addr virt:0x%lx, phys addr: 0x%lx\n",
...
static int sprd_dma_alloc_chan_resources(struct dma_chan *chan)
{
	...
	pr_err(DMA_TAG "%s Alloc chan[%d] resources is OK, chn_reg=0x%lx,\n",
...
static void sprd_dma_free_chan_resources(struct dma_chan *chan)
{
	...
	pr_err(DMA_TAG "%s release chan [%d] resources is OK!\n",
		dma_chan_name(chan), mchan->chan_num);
#ifdef CONFIG_PM
...
#endif
	pr_err(DMA_TAG "All DMA chns were released, disable DMA clock!\n");
}
