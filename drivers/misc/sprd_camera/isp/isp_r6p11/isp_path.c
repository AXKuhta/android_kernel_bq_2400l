...
#if 0
static int isp_get_storecce_param(struct isp_offline_desc *store_cce)
{
	struct isp_store_info *store_info = NULL;
...
static int isp_set_fetch(unsigned int idx, struct isp_path_desc *path)
{
	...
	/* TODO: configured by dcam*/
	enum isp_fetch_format clr_fmt = ISP_FETCH_CSI2_RAW_10;
	...
	default:
		pr_err("error: invalid color fmt %d\n", clr_fmt);
		return -1;
	}
