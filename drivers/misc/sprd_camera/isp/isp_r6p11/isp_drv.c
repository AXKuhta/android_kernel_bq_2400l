...
static int wait_for_full_tx_done(struct isp_pipe_dev *dev)
{
	...
	while (flag != DCAM_TIME_TX_DONE) {
		if (flag == DCAM_TIME_NULL) {
			pr_err("failed to wait for full path tx done!\n");
			return -EFAULT;
		}
		usleep_range(1000, 1100);
		flag = atomic_read(&dcam_full_path_time_flag);
		if (loop++ > WAIT_CNT_MS) {
			pr_err("error: wait for full tx done TimeOut!\n");
			/*
			 * FIXME: maybe need exception process here,
			 * when waiting time_out
			 * <goto time_out_exit;>
...
static int sprd_isp_pipeline_proc_bin(void *handle)
{
	...
	pr_debug("valid bin path frame, fetch_addr:0x%08xn",
		 valid_frame->yaddr_vir);
...
static int iommu_get_addr_by_path(struct isp_module *module, enum isp_scl_id path_id)
{
	enum dcam_drv_rtn rtn = ISP_RTN_SUCCESS;
	struct camera_frame *reserved_frame = NULL;

	reserved_frame = &module->path_reserved_frame[path_id];
	if (NULL != reserved_frame && reserved_frame->pfinfo.iova[0] == 0) {
...
static void iommu_release_addr_by_path(struct isp_module *module, enum isp_scl_id path_id)
{
	struct camera_frame *reserved_frame = NULL;

	reserved_frame = &module->path_reserved_frame[path_id];

	if (NULL != reserved_frame) {
	...
	enum isp_cfg_id id, void *param)
{
	...
	case ISP_PATH_SN_MAX_SIZE:
		...
		buf_desc->buf_len =
			ALIGN((unsigned long)((size->w * size->h) * 5 / 4),
			      ISP_PIXEL_ALIGN_WIDTH);
		break;
...
int sprd_isp_set_offline_buffer(void *isp_handle,
	uint8_t off_type, struct camera_frame *out_frame)
{
	int rtn = ISP_RTN_SUCCESS;
	struct camera_frame frame;
	...
	if (off_desc->read_buf_err != 1) {
#if 0 /* TODO: removed for bringup */
		if (off_desc->shadow_done_cnt < 1 &&
			dev->is_raw_capture != 1) {
			pr_debug("offline shadow_done_cnt %d\n",
				off_desc->shadow_done_cnt);
			rtn = ISP_RTN_PATH_ADDR_ERR;
			goto _exit;
		}
#endif
		...
		if (isp_frame_dequeue(&buf_desc->frame_queue, &frame)) {
			pr_err("offline producer (%d) dequeue error\n",
				off_type);
			rtn = ISP_RTN_PATH_ADDR_ERR;
			goto _exit;
		}
		...
		if (isp_frame_enqueue(&buf_desc->zsl_queue, &frame)) {
			pr_err("offline consumer (%d) enqueue error\n",
				off_type);
			rtn = ISP_RTN_PATH_ADDR_ERR;
			goto _exit;
		}
		...
		if (off_type == ISP_OFF_BUF_BIN) {
			rtn = sprd_isp_start_pipeline_bin(isp_handle,
							  DCAM_CAPTURE_NONE);
			if (rtn) {
				pr_err("start isp pipeline bin path error\n");
				rtn = ISP_RTN_PATH_ADDR_ERR;
				goto _exit;
			}
			...
			rtn = sprd_isp_start_pipeline_full(isp_handle,
							   DCAM_CAPTURE_NONE);
			if (rtn) {
				pr_err("start isp pipeline full path error\n");
				rtn = ISP_RTN_PATH_ADDR_ERR;
				goto _exit;
			}
		} else {
			pr_err("invalid off_type\n");
			rtn = EINVAL;
			goto _exit;
		}
	} else
