	...
	for (num = 0; num < frm_q_len; num++) {
		...
		if (sprd_iommu_attach_device(&s_isp_pdev->dev) == 0) {
			...
			if (IS_ERR(ion_buf->handle)) {
				pr_err("failed to alloc offline tmp buf\n");
				goto _err_destroy;
			}
		} else {
			ion_buf->handle = ion_alloc(ion_buf->client,
			...
			pr_info("offline_buf reserved: pa:0x%lx, len:0x%zx, client=%p, hd=%p(%x)\n",
				phys_addr, buf_len,
	...
	for (num = 0; num < frm_q_len; num++) {
		...
		if (ion_buf->addr.yaddr_vir) {
			...
			rtn = sprd_iommu_unmap(&s_isp_pdev->dev,
						    &iommu_data);
			if (rtn) {
				pr_err("failed to unmap isp iommu addr\n");
				return -EFAULT;
			}
	...
	for (num = 0; num < frm_q_len; num++) {
		...
		if (ion_buf->addr.uaddr_vir) {
			...
			rtn = sprd_iommu_unmap(&s_dcam_pdev->dev,
				&iommu_data);
			if (rtn) {
				pr_err("failed to unmap dcam iommu addr\n");
				return -EFAULT;
			}
...
int isp_gen_buf_alloc(struct isp_buf_info *buf_info)
{
	...
	if (IS_ERR(buf_info->client)) {
		pr_err("failed to create isp buf ION client\n");
		rtn = EPERM;
	...
	if (IS_ERR(buf_info->handle)) {
		pr_err("failed to alloc isp buf\n");
		rtn = EPERM;
		goto _err_destroy;
	}
	...
	if (iommu_enabled == 0) {
		rtn = ion_phys(buf_info->client, buf_info->handle,
			     ...
			     &buf_info->size);
		if (rtn) {
			pr_err("failed to get phys, err=%d\n", rtn);
...
int isp_gen_buf_hw_map(struct isp_buf_info *buf_info)
{
	...
	if (!buf_info->hw_addr) {
		...
		rtn = sprd_iommu_map(&s_isp_pdev->dev, &iommu_data);
		if (rtn) {
			pr_err("failed to get iova, err=%d\n", rtn);
			rtn = -EFAULT;
			goto _exit;
		}
...
err_exit:
	pr_err("failed to recycle buf, args error!\n");
	return ret;
}
