...
static int sprd_img_check_aem_path_cap(unsigned int fourcc,
				    struct sprd_img_format *f,
				    struct camera_info *info)
{
	struct camera_path_spec *aem_path = &info->dcam_path[CAMERA_AEM_PATH];

	DCAM_TRACE("check format for aem path\n");

	aem_path->frm_type = f->channel_id;
	aem_path->is_work = 0;
	/* need add aem path necessary info here*/
	...
	case IMG_PIX_FMT_RGB565X:
		if (info->sn_mode == DCAM_CAP_MODE_RAWRGB) {
			if (path->is_from_isp) {
				...
				pr_err("fail to support, RawRGB sensor, no ISP, format 0x%x\n",
				       fourcc);
				return -EINVAL;
			}
		} else {
...
static int sprd_img_binning_tx_done(struct camera_frame *frame, void *param)
{
	...
	ret = sprd_isp_set_offline_buffer(dev->isp_dev_handle,
		ISP_OFF_BUF_BIN, frame);
	dev->bin_frame_id++;
	/* TODO: binning offline buf should different from full */
	DCAM_TRACE_INT("binning path tx done dcam%d, 0x%08X\n",
			dev->idx, frame->yaddr_vir);
	...
	enum isp_path_index path_index)
{
	...
	ret = set_dcam_cap_cfg(idx, DCAM_CAP_BAYER_PATTERN, &info->img_ptn);
	if (unlikely(ret)) {
			      ...
			      struct sprd_img_format *img_format)
{
	...
	} else if (img_format->channel_id == CAMERA_AEM_PATH) {
		mutex_lock(&dev->dcam_mutex);
		ret = sprd_img_check_aem_path_cap(fmt->fourcc,
					       img_format,
					       &dev->dcam_cxt);
		mutex_unlock(&dev->dcam_mutex);
		channel_id = CAMERA_AEM_PATH;
	} else {
		mutex_lock(&dev->dcam_mutex);
