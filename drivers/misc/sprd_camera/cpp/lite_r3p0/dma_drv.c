					...
					struct dma_drv_private *p)
{
	...
	if ((p->src_offset_x % 2 != 0) || (p->dst_offset_x % 8 != 0)) {
		pr_err("fail to src and dest offset x:%u, %u\n",
	...
	if ((p->src_pitch % 8 != 0)
		...
		pr_err("fail to align src pitch:%u\n", p->src_pitch);
		return -EINVAL;
	}

	if ((p->dst_pitch % 8 != 0)
		|| (p->dst_pitch < (parm->input_size.w + p->dst_offset_x))) {
		pr_err("fail to align dest pitch:%u\n", p->src_pitch);
		return -EINVAL;
	}
...
static int cpp_dma_set_parm(struct dma_drv_private *p)
{
	...
	if ((p->dma_src_addr & DMA_ADDR_ALIGN) ||
		...
		pr_err("fail to align src and dst addr\n");
		return -EINVAL;
	}
