			...
			cpp_isr_func user_func, void *priv)
{
	...
	if (id < CPP_IRQ_NUMBER) {
		...
		pr_err("fail to get cpp isr irq\n");
	}
}
...
{
	pr_err("fail to use memory: cpp MMU error!!!\n");
}
#endif
			...
			unsigned long arg)
{
	...
	case SPRD_CPP_IO_START_ROT:
	{
		rotif = dev->rotif;
		if (!rotif) {
			pr_err("fail to start rot, rotif is null!\n");
			return -EINVAL;
		}
	...
	case SPRD_CPP_IO_START_SCALE:
	{
		scif = dev->scif;
		if (!scif) {
			pr_err("fail to start scale, scif is null!\n");
			return -EFAULT;
		}
	...
	case SPRD_CPP_IO_START_DMA:
	{
		dmaif = dev->dmaif;
		if (!dmaif) {
			pr_err("fail to start dma, dmaif is null!\n");
			return -EFAULT;
		}
	...
	default:
		pr_err("fail to get cpp inval cmd %d\n", _IOC_NR(cmd));
		break;
	}
