...
static int sprd_sensor_io_get_id0(struct sprd_sensor_file_tag *p_file,
				 unsigned long arg)
{
	int ret = 0;
	unsigned char power_level;
	...
	ret = copy_from_user(&power_level, (unsigned char *)arg,
			     sizeof(unsigned char));
	if (ret == 0)
		value  = sprd_sensor_get_sensorid0(p_file->sensor_id);
	printk("sprd_sensor_io_get_id0 %d\n",value);
	return value;
}

static int sprd_sensor_io_get_id1(struct sprd_sensor_file_tag *p_file,
				 unsigned long arg)
{
	int ret = 0;
	unsigned char power_level;
	...
	ret = copy_from_user(&power_level, (unsigned char *)arg,
			     sizeof(unsigned char));
	if (ret == 0)
		value  = sprd_sensor_get_sensorid1(p_file->sensor_id);
	printk("sprd_sensor_io_get_id1 %d\n",value);
	return value;
}
static int sprd_sensor_io_get_id2(struct sprd_sensor_file_tag *p_file,
				 unsigned long arg)
{
	int ret = 0;
	unsigned char power_level;
	...
	ret = copy_from_user(&power_level, (unsigned char *)arg,
			     sizeof(unsigned char));
	if (ret == 0)
		value  = sprd_sensor_get_sensorid2(p_file->sensor_id);
	printk("sprd_sensor_io_get_id2 %d\n",value);
	return value;
}
				   ...
				   unsigned long arg)
{
	...
	case SENSOR_IO_POWER_CFG:
		...
		break;
	case SENSOR_IO_GET_CAMERAID0:
		ret = sprd_sensor_io_get_id0(p_file, arg);
		printk("sprd_sensor_file_ioctl getid0 = %d\n",ret);
		break;
	case SENSOR_IO_GET_CAMERAID1:
		ret = sprd_sensor_io_get_id1(p_file, arg);
		printk("sprd_sensor_file_ioctl getid1 = %d\n",ret);
		break;
	case SENSOR_IO_GET_CAMERAID2:
		ret = sprd_sensor_io_get_id2(p_file, arg);
		printk("sprd_sensor_file_ioctl getid2 = %d\n",ret);
		break;
	}
