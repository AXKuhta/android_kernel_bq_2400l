...
/* for wakelock */
void sdiohal_lock_tx_wakelock(void)
{
	struct sdiohal_data_t *p_data = sdiohal_get_data();
	...
	wake_lock(&p_data->tx_wakelock);
}

void sdiohal_unlock_tx_wakelock(void)
{
	struct sdiohal_data_t *p_data = sdiohal_get_data();
	...
	wake_unlock(&p_data->tx_wakelock);
}

void sdiohal_lock_rx_wakelock(void)
{
	struct sdiohal_data_t *p_data = sdiohal_get_data();
	...
	wake_lock(&p_data->rx_wakelock);
}

void sdiohal_unlock_rx_wakelock(void)
{
	struct sdiohal_data_t *p_data = sdiohal_get_data();
	...
	wake_unlock(&p_data->rx_wakelock);
}
...
void sdiohal_wakelock_init(void)
{
	...
	wake_lock_init(&p_data->tx_wakelock,
		WAKE_LOCK_SUSPEND, "sdiohal_tx_wakelock");
	wake_lock_init(&p_data->rx_wakelock,
		WAKE_LOCK_SUSPEND, "sdiohal_rx_wakelock");
}
...
void sdiohal_cp_tx_sleep(void)
{
	...
	slp_mgr_drv_sleep(SLP_TX, true);
}

void sdiohal_cp_tx_wakeup(void)
{
	sdiohal_debug("%s enter\n", __func__);
	slp_mgr_drv_sleep(SLP_TX, false);
	slp_mgr_wakeup(SLP_TX);
}

void sdiohal_cp_rx_sleep(void)
{
	sdiohal_debug("%s enter\n", __func__);
	slp_mgr_drv_sleep(SLP_RX, true);
}

void sdiohal_cp_rx_wakeup(void)
{
	sdiohal_debug("%s enter\n", __func__);
	slp_mgr_drv_sleep(SLP_RX, false);
	slp_mgr_wakeup(SLP_RX);
}
#endif
...
void sdiohal_resume_check(void)
{
	...
	while (!atomic_read(&p_data->flag_resume)) {
		if (cnt == 0) {
			sdiohal_err("sdiom, wait sdio resume %s\n", __func__);
			dump_stack();
		}
...
void sdiohal_resume_wait(void)
{
	...
	while (!atomic_read(&p_data->flag_resume)) {
		sdiohal_err("sdiom, sleep 5ms wait for sdio resume\n");
		usleep_range(4000, 6000);
	}
}
...
struct sdiohal_list_t *sdiohal_get_rx_mbuf_list(int num)
{
	...
	for (i = 0; i < num; i++) {
		mbuf_temp->buf = sdiohal_alloc_frag(MAX_PAC_SIZE,
...
static int sdiohal_eof_buf_init(void)
{
	...
	p_data->eof_buf = kzalloc(MAX_PAC_SIZE, GFP_KERNEL);
	if (!p_data->eof_buf)
		return -ENOMEM;
...
static int sdiohal_dtbs_buf_init(void)
{
	...
	p_data->dtbs_buf = sdiohal_alloc_frag(MAX_PAC_SIZE,
					      GFP_ATOMIC | __GFP_COLD);
	if (!p_data->dtbs_buf)
		return -ENOMEM;
