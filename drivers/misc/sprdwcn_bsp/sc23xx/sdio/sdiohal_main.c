...
static void sdiohal_abort(void)
{
	...
	sdiohal_info("sdiohal:sdiohal_abort\n");
...
static void sdiohal_success_trans_pac_num(void)
{
	...
	sdiohal_info("sdiohal:success num:[%d]\n",
	...
	struct sdio_func *sdio_func, uint fix_inc, bool dir, uint addr)
{
	...
	if (dir) {
		sg_data_size = SDIOHAL_ALIGN_512BYTE(ttl_len +
		...
		ttl_len = SDIOHAL_ALIGN_512BYTE(ttl_len
...
int sdiohal_scan_card(void)
{
	...
	sdiohal_info("sdiohal:sdiohal_scan_card\n");

	if (p_data->sdio_dev_host == NULL) {
		sdiohal_err("sdiohal:sdio_sdio_rescan get host failed!");
		return -1;
	}
...
void sdiohal_set_carddump_status(unsigned int flag)
{
	...
	sdiohal_info("sdiohal:carddump flag set[%d]\n", flag);
	if (flag == true) {
		disable_irq(p_data->irq_num);
		sdiohal_info("sdiohal:disable rx int for dump\n");
	}
...
static int sdiohal_get_dev_func(struct sdio_func *func)
{
	...
	if (func->num >= SDIOHAL_MAX_FUNCS) {
		sdiohal_err("sdiohal:func num err!!! func num is %d!!!",
			func->num);
		return -1;
	}
	sdiohal_info("sdiohal:func num is %d!!!", func->num);
...
static int sdiohal_parse_dt(void)
{
	...
	if (sdio_node == NULL) {
		sdiohal_err("sdiohal get sdhci-name failed");
		return -1;
	}
	p_data->sdio_dev_host = sdiohal_dev_get_host(sdio_node);
	if (p_data->sdio_dev_host == NULL) {
		sdiohal_err("sdiohal get host failed!!!");
		return -1;
	}
	sdiohal_info("sdiohal: get host ok!!!");
	...
	const struct sdio_device_id *id)
{
	...
	if (ret < 0) {
		sdiohal_err("sdiohal:get func err!!!\n");
		return ret;
	}
	sdiohal_info("sdiohal:get func ok!!!\n");
	...
	if (p_data->pwrseq_disable) {
		...
		sdio_release_host(p_data->sdio_func[FUNC_1]);
		if (ret < 0) {
			sdiohal_err("enable func1 err!!! ret is %d", ret);
			return ret;
		}
		sdiohal_info("enable func1 ok!!!");
	...
	ret = devm_request_irq(&func->dev, p_data->irq_num,
			       sdiohal_irq_handler,
	...
	if (scan_card_notify != NULL)
		...
		sdiohal_err("sdiohal:rescan callback not exist\n");
...
static void sdiohal_launch_thread(void)
{
	...
	if (tx_thread)
		...
		sdiohal_err("sdiohal create sdiohal_tx_thread fail\n");
		return;
	}
	...
	if (rx_thread)
		...
		sdiohal_err("sdiohal creat sdiohal_rx_thread fail\n");
}
