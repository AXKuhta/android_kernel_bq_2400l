...
long int sdiohal_log_level = SDIOHAL_NORMAL_LEVEL | SDIOHAL_LIST_LEVEL |
			     SDIOHAL_DATA_LEVEL | SDIOHAL_PERF_LEVEL;

int sdiohal_extract_num(char *p, int cur)
{
	int i, num = 0;
...
int sdiohal_simple_test_tx(size_t count)
{
...
int sdiohal_simple_test_rx(void)
{
	return 0;
}

int sdiohal_throughput_test_tx(void)
{
	struct mbuf_t *head, *tail, *temp;
...
int sdiohal_throughput_test_rx(void)
{
	return 0;
}
		   ...
		   struct mbuf_t *tail, int num)
{
	...
	if (tp_tx_flag == 1) {
		...
		if (tp_tx_cnt % 500 == 0) {
			...
			sdiohal_info("sdiohal tx -> times(500c) is %lld\n",
				times_count);
			do_gettimeofday(&tp_tx_start_time);
		}

		sdiohal_throughput_test_tx();
	}
...
#if TP_TEST_TX
struct mchn_ops_t tp_tx_ops = {
	.channel = AT_TX_CHANNEL,
	.hif_type = HW_TYPE_SDIO,
	.inout = SDIOHAL_DIR_TX,
	.pool_size = TP_TX_BUF_CNT,
	.pop_link = at_list_tx_pop,
};
#endif

#if TP_TEST_RX
struct mchn_ops_t tp_rx_ops = {
	.channel = AT_RX_CHANNEL,
	.hif_type = HW_TYPE_SDIO,
	.inout = SDIOHAL_DIR_RX,
	.pool_size = 1,
...
#if TP_TEST_TX
	sprdwcn_bus_chn_init(&tp_tx_ops);
#endif
#if TP_TEST_RX
	sprdwcn_bus_chn_init(&tp_rx_ops);
#endif
...
#if TP_TEST_TX
	sprdwcn_bus_chn_deinit(&tp_tx_ops);
#endif
#if TP_TEST_RX
	sprdwcn_bus_chn_deinit(&tp_rx_ops);
#endif
