...
static int gnss_dump_cp_register_data(u32 addr, u32 len)
{
	...
	if (!iram_buffer) {
		pr_err("%s vmalloc iram_buffer error\n", __func__);
		return -ENOMEM;
	}
...
{
	struct regmap *regmap;
	u32 value[9] = {0}; /* value[0]board version + value[1:8] reg value */
	u32 i = 0;
	mm_segment_t fs;
	u32 len = 0;
	u8 *ptr = NULL;
	...
	if (wcn_platform_chip_type() == WCN_PLATFORM_TYPE_SHARKLE) {
		...
		len = sizeof(gnss_sharkle_ap_reg);
	} else {
		gnss_ap_reg = gnss_pike2_ap_reg;
		len = sizeof(gnss_pike2_ap_reg);
	}
...
static int gnss_dump_share_memory(u32 len)
{
	...
	if (!ddr_buffer) {
		pr_err(" %s vmalloc ddr_buffer fail\n", __func__);
		return -1;
	}
