...
static ssize_t keypad_disable_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
		unsigned char data = 0;
	  return sprintf(buf, "%d\n", data);
...
{
	keypab_ca_enable=1;
	printk("enable_keypad_irq\n");
	return count;
}
static ssize_t keypad_disable_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	keypab_ca_enable=0;
	printk("disable_keypad_irq\n");
	return count;
}
static DEVICE_ATTR(enableirq, S_IWUSR | S_IRUGO,
		   keypad_enable_show,
		   keypad_enable_store);

static DEVICE_ATTR(disableirq, S_IWUSR | S_IRUGO,
		   keypad_disable_show,
		   keypad_disable_store);



static struct attribute *keypad_attributes[] = {
	&dev_attr_enableirq.attr,
	&dev_attr_disableirq.attr,
	NULL
};


static struct attribute_group keypad_attribute_group = {
	.attrs = keypad_attributes
};
...
static int sprd_keypad_probe(struct platform_device *pdev)
{
  ...
  error = sysfs_create_group(&pdev->dev.kobj, &keypad_attribute_group);
  if (error) {
