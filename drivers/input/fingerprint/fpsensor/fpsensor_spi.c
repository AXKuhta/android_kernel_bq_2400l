...
static long fpsensor_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case FPSENSOR_IOC_INIT:
		...
		if (retval == 0)
			pr_info("irq thread reqquest success\n");
		else
			pr_info("irq thread retval =%d\n", retval);
		...
		pr_info("fpsensor init finished======\n");
		break;
	...
	case FPSENSOR_IOC_EXIT:
		...
		pr_info("fpsensor exit finished======\n");
		break;
	...
	case FPSENSOR_IOC_ENABLE_IRQ:
		pr_info("chip ENable IRQ command\n");
		fpsensor_enable_irq(fpsensor_dev);
		break;

	case FPSENSOR_IOC_DISABLE_IRQ:
		pr_info("chip disable IRQ command\n");
		fpsensor_disable_irq(fpsensor_dev);
		break;
	...
	case FPSENSOR_IOC_INPUT_KEY_EVENT:
		...
		pr_info("received key event[%d], key=%d, value=%d\n",
...
static int fpsensor_probe(struct platform_device *pdev)
{
	...
	pr_info("Setting fpsensor device configuration.\n");
	...
	pr_info("probe finished, normal driver version: %s\n",
