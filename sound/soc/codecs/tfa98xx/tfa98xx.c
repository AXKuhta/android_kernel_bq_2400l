...
struct tfa98xx * temp_tfa98xx = NULL;
static int tfa98xx_load_container(struct tfa98xx *tfa98xx);
static int tfa98xx_load_container_nowait(struct tfa98xx *tfa98xx);
...
static int firmware_filectrl_show(struct seq_file *n, void *v){
    return 0;
}

static int firmware_filectrl_open(struct inode *inode, struct file *file) {
    return single_open(file,firmware_filectrl_show,inode->i_private);
}

static int tfa98xx_ctrl_write(struct file *filep, const char *ubuf, size_t cnt, loff_t *data){
    char buf[64];
    long value;
    int ret = 0;
    ...
    if(copy_from_user(buf,ubuf,cnt))
        return -EINVAL;
    buf[cnt] = 0;
    ret = kstrtoul(buf,10,(unsigned long *)&value);
    if(ret < 0) {
        return ret;
    }
    pr_info("%s, buf:%s, value %ld, called by rc node\n", __func__, buf, value);

     switch(value) {
        case 1:
            if(temp_tfa98xx)
            ret = tfa98xx_load_container_nowait(temp_tfa98xx);
            break;
        default:
            break;
     }
...
static const struct file_operations load_firmware = {
    .open = firmware_filectrl_open,
    .write = tfa98xx_ctrl_write,
    .read = seq_read,
    .llseek = seq_lseek,
    .release = single_release,
};

static int  tfa98xx_sprd_init_ctrl(struct tfa98xx * tfa98xx) {
    struct proc_dir_entry *pe;
    struct proc_dir_entry *tfa98xx_dir = NULL;
    temp_tfa98xx = tfa98xx;
    tfa98xx_dir = proc_mkdir("tfa98xx",NULL);
    if(!tfa98xx_dir) {
        pr_err("%s, create /proc/tfa98xx  failed \n", __func__);
        return -ENOMEM;
    }
    pe = proc_create("tfa98xx_fw_update",0664,tfa98xx_dir, &load_firmware);
    if(!pe) {
        return -ENOMEM;
    }
    return 0;
...
static int tfa98xx_load_container_l(struct tfa98xx *tfa98xx);

static int firmware_load_get(struct snd_kcontrol *kcontrol,
			   struct snd_ctl_elem_value *ucontrol)
{
    struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
    struct tfa98xx *tfa98xx = snd_soc_codec_get_drvdata(codec);

    ucontrol->value.integer.value[0] = tfa98xx->firmware_load_state;

	return 0;
}

static int firmware_load_put(struct snd_kcontrol *kcontrol,
			   struct snd_ctl_elem_value *ucontrol)
{
	int ret;
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	...
	pr_info("%s, %s, called by control\n", __func__,
			texts->texts[ucontrol->value.integer.value[0]]);

	ret = ucontrol->value.integer.value[0];
	if (ret == 1)
		ret = tfa98xx_load_container_l(tfa98xx);

	return 0;
}

static const char *const firmware_function[] = {
	"idle", "loading", "loaded",
};

static const struct soc_enum firmware_load =
	SOC_ENUM_SINGLE_EXT(3, firmware_function);

static int tfa_imped_value = 0;
...
static int tfa98xx_cali_ctrl_get(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct tfa98xx *tfa98xx = snd_soc_codec_get_drvdata(codec);
	...
	struct tfa98xx_calibratedata calibratedata;
	int calibrateDone = 0;
	...
	pr_info("%s tfa_imped_value %d\n", __func__,tfa_imped_value);

	i2c = tfa98xx->i2c;
	memset(&calibratedata, 0x00, sizeof(struct tfa98xx_calibratedata));

	mutex_lock(&tfa98xx->dsp_lock);
	ret = tfa98xx_open(tfa98xx->handle);
	if (ret){
		pr_err("run API tfa98xx_open() returned error(%d)\n", ret);
		goto err;
	}else{
		/* check calibrateion status. */
		ret = tfaRunWaitCalibration(tfa98xx->handle, &calibrateDone);
		if (ret){
			pr_err("%s, run API tfaRunWaitCalibration() returned error(%d)	calibrateDone=%d\n",__func__, ret, calibrateDone);
			ret = -ret;
			goto err;
		}
		if (1 != calibrateDone){
		...
		/* get speaker count */
		ret = tfa98xx_supported_speakers(tfa98xx->handle, &calibratedata.spk_count);
		pr_info("%d %s \n",__LINE__, __func__);
		if (ret){
			pr_err("%s, run API tfa98xx_supported_speakers() returned error(%d)\n",__func__, ret);
			goto err;
		}
		pr_info("%d %s spk_count %d\n", __LINE__,__func__,calibratedata.spk_count);

		/* get impedance data.*/
		ret = tfa_dsp_get_calibration_impedance(tfa98xx->handle);
		if (ret){
			pr_err("%s, run API tfa_dsp_get_calibration_impedance() returned error(%d)\n",__func__, ret);
			goto err;
		}else{
			int i = 0;
			for(i=0; i<calibratedata.spk_count; i++){
				calibratedata.impedance[i] = tfa_get_calibration_info(tfa98xx->handle, i);
				pr_info("%d %s,spk_count %d,impedance[%d] %d \n",
					__LINE__, __func__, calibratedata.spk_count,i,calibratedata.impedance[i]);
			}
			if (calibratedata.spk_count == 1)
				sprintf(buff, "tfa cali imped %d\n", calibratedata.impedance[0]);
			if (calibratedata.spk_count == 2)
				sprintf(buff, "tfa cali imped %d + %d\n", calibratedata.impedance[0],calibratedata.impedance[1]);
			tfa_imped_value = calibratedata.impedance[0];
			ucontrol->value.integer.value[0] = calibratedata.impedance[0];
			pr_info("%s: value is %s, tfa_imped_value %d\n", __func__, buff, tfa_imped_value);
		}

	}

err:
	pr_info("%s: value is %s, ret=%d\n", __func__, buff, ret);
	tfa98xx_close(tfa98xx->handle);

	/* if we get error, clear the output data buffer. */
	if (ret){
		memset(&calibratedata, 0x00, sizeof(struct tfa98xx_calibratedata));
	}
	...
	if(ret){
		...
		//return -EINVAL;//-ret;//these values return let this kcontrol display abnormal when run tinymix
		return 0;
	}else{
		pr_info("%s,buff %s,tfa_imped_value %d, exit\n", __func__,buff,tfa_imped_value);
		return 0;
	}
}

static int tfa98xx_cali_ctrl_set(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct tfa98xx *tfa98xx = snd_soc_codec_get_drvdata(codec);
	...
	pr_info("%s,entry, tfa_imped_value %d, set to %ld\n", __func__,tfa_imped_value,
			ucontrol->value.integer.value[0]);
	if (NULL == tfa98xx){
		pr_err("%s	  tfa98xx is NULL.\n", __func__);
		return -EINVAL;
	...
	ret = tfa98xx_open(tfa98xx->handle);
	if (ret){
		pr_err("%s run API tfa98xx_open() returned error(%d)\n",__func__, ret);
		goto err;
	}else{
		ret = tfa98xx_dsp_system_stable(tfa98xx->handle, &ready);
		if (ret){
			pr_err("%s run API tfa98xx_dsp_system_stable() returned error(%d)\n",__func__, ret);
			goto err;
		}
		if (0 == ready){
			pr_err("%s run API tfa98xx_dsp_system_stable() returned ready=%d\n",__func__, ready);
		...
		if (ret){
			pr_err("%s run API tfaRunStartup() returned error(%d)\n",__func__, ret);
			goto err;
		}

		ret = tfaRunColdboot(tfa98xx->handle, 1);
		if (ret){
			pr_err("%s run API tfaRunColdboot() returned error(%d)\n",__func__, ret);
			goto err;
		}
		...
		if(ret){
			pr_err("%s run API tfa98xx_set_mtp() clear MTPEX returned error(%d)\n",__func__, ret);
			goto err;
		}

		/* set MTPOTC flag */
		if(0 == strncmp(kernelbuf, "once", strlen("once"))){
		...
		if (ret){
			pr_err("%s run API tfa98xx_set_mtp() set MTPOTC returned error(%d)\n",__func__, ret);
			goto err;
		}

		ret = tfaRunStartDSP(tfa98xx->handle);
		if (ret){
			pr_err("%s run API tfaRunStartDSP() returned error(%d)\n",__func__, ret);
			goto err;
		}
		ret = tfaContWriteFiles(tfa98xx->handle);
		if (ret){
			pr_err("%s run API tfaContWriteFiles() returned error(%d)\n",__func__, ret);
			goto err;
		}
		ret = tfaContWriteFilesProf(tfa98xx->handle, tfa98xx_profile, 0);
		if (ret){
			pr_err("%s, run API tfaContWriteFilesProf() returned error(%d)\n",__func__, ret);
			goto err;
		}
		...
		if(ret){
			...
			pr_info("%s Device 0x%x new impedance is %d mohm\n", __func__,
				i2c->addr, tfa_get_calibration_info(tfa98xx->handle, 0));

			ret = tfa_set_filters(tfa98xx->handle, tfa98xx_profile);
			if(ret){
				pr_err("%s, run API tfa_set_filters() returned error %d\n",__func__, ret);
			}

			tfa_set_swprof(tfa98xx->handle, tfa98xx_profile);
			ret = tfa98xx_set_mute(tfa98xx->handle, Tfa98xx_Mute_Off);
			if(ret){
				pr_err("%s, run API tfa98xx_set_mute() returned error %d\n",__func__, ret);
			}
		}
	}
...
err:
	...
	if (ret){
		pr_err("%s, Device %x Calibration failed!!!, ret %d\n",__func__, i2c->addr, ret);
		return -EINVAL;
	}else{
		pr_info("%s, Device %x calibration completed successfully\n",__func__, i2c->addr);
		return 0;
	}
}

static const struct snd_kcontrol_new  tfa_controls[] = {
	SOC_ENUM_EXT("TFA Firmware Load", firmware_load, firmware_load_get,
		     firmware_load_put),
	SOC_SINGLE_EXT("TFA Imped Cali",  0, 0, 65530, 0,
		tfa98xx_cali_ctrl_get, tfa98xx_cali_ctrl_set),
};
	...
	if (tfa98xx->handle == 0) {
		...
		if (ret == Tfa98xx_Error_Ok) {
			...
			ret = -1;
				...
				pr_err("dll+++++ret == ret unknow result\n");
			}
		mutex_unlock(&tfa98xx->dsp_lock);
	}
	pr_info("%d %s \n", __LINE__, __func__);
...
static void tfa98xx_container_firmware_loaded(const struct firmware *cont, void *context)
{
	int ret = 0;
	struct tfa98xx *tfa98xx = context;
	mutex_lock(&tfa98xx->tfa98xx_mutex);
	pr_info("%s: tfa98xx->firmware_load_ok:%d \n", __func__, tfa98xx->firmware_load_ok);
	if(!tfa98xx->firmware_load_ok) {
		tfa98xx->dsp_fw_state = TFA98XX_DSP_FW_PENDING;
		ret = tfa98xx_container_loaded(cont, tfa98xx);
		if(!ret) {
			tfa98xx->firmware_load_ok = 1;
			pr_info("%s: tfa98xx_container_loaded ok:ret:%d\n", __func__, ret);
		} else {
			pr_err("%s tfa98xx_container_loaded fail %d, called by rc node\n", __func__,ret);
		}
		release_firmware(cont);
	}
	mutex_unlock(&tfa98xx->tfa98xx_mutex);
}
...
static int tfa98xx_load_container(struct tfa98xx *tfa98xx)
{
	...
	int rc = 0;
	mutex_lock(&tfa98xx->tfa98xx_mutex);
	pr_err("%s in, tfa98xx->firmware_load_ok:%d, called by control\n",
		__func__, tfa98xx->firmware_load_ok);
	if(!tfa98xx->firmware_load_ok) {
		pr_info("%d %s called by control\n", __LINE__, __func__);
		tfa98xx->dsp_fw_state = TFA98XX_DSP_FW_PENDING;
		memset(tfa98xx->firmware_path, 0,
		sizeof(tfa98xx->firmware_path));
		strcpy(tfa98xx->firmware_path, AUDIO_FIRMWARE_PATH_BASE);
		strcat(tfa98xx->firmware_path,fw_name);
		rc = audio_load_firmware_data(&firmware, tfa98xx->firmware_path);
		if (rc <0) {
			mutex_unlock(&tfa98xx->tfa98xx_mutex);
			audio_release_firmware_data(&firmware);
			return rc;
		}
		pr_info("%d %s called by control\n", __LINE__, __func__);
		rc = tfa98xx_container_loaded(&firmware, tfa98xx);
		if(!rc) {
			pr_info("%d, %s,tfa98xx_container_loaded ok, called by control\n", __LINE__, __func__);
			tfa98xx->firmware_load_ok = 1;
		} else {
			pr_info("%d, %s,called by control fail\n", __LINE__, __func__);
		}
		audio_release_firmware_data(&firmware);
	}
	mutex_unlock(&tfa98xx->tfa98xx_mutex);
...
static int tfa98xx_load_container_nowait(struct tfa98xx *tfa98xx)
{
	pr_info("%s: called by rc node\n", __func__);
	return request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
				       fw_name, tfa98xx->dev, GFP_KERNEL,
				       tfa98xx, tfa98xx_container_firmware_loaded);
}

static void tfa98xx_firmware_load_work(struct work_struct *work)
{
    struct tfa98xx *tfa98xx = container_of(work, struct tfa98xx, firmware_work.work);
    int ret = 0;
    if (tfa98xx) {
        pr_err("%d %s, called by control\n", __LINE__, __func__);
        ret = tfa98xx_load_container(tfa98xx);
        if (ret < 0) {
           pr_err("ERR:Failed to load firmware, ret:%d\n", ret);
        } else {
            pr_info("%s, load_firmware_work load firmware ok ret:%d\n", __func__, ret);
        }
    }
}
...
static int tfa98xx_load_container_l(struct tfa98xx *tfa98xx)
{
	pr_info("%s in, called by control\n", __func__);
	queue_delayed_work(tfa98xx->tfa98xx_wq, &tfa98xx->firmware_work, 0);
	return 0;
}
						...
						struct snd_soc_dai *dai)
{
	...
	pr_info("%s: tfa98xx->dsp_fw_state %d \n", __func__, tfa98xx->dsp_fw_state);
	/*
...
static int tfa98xx_probe(struct snd_soc_codec *codec)
{
	...
	INIT_DELAYED_WORK(&tfa98xx->firmware_work, tfa98xx_firmware_load_work);
	tfa98xx->codec = codec;
	mutex_init(&tfa98xx->tfa98xx_mutex);
	ret = tfa98xx_load_container_nowait(tfa98xx);
	tfa98xx_sprd_init_ctrl(tfa98xx);
...
static int tfa98xx_remove(struct snd_soc_codec *codec)
{
	...
	cancel_delayed_work_sync(&tfa98xx->init_work);
	cancel_delayed_work_sync(&tfa98xx->tapdet_work);
	cancel_delayed_work_sync(&tfa98xx->firmware_work);
	if (tfa98xx->tfa98xx_wq)
...
#endif
	.controls = tfa_controls,
	.num_controls = ARRAY_SIZE(tfa_controls),
};
