...
static void write_root_stat_emmc(void)
{
	...
	rln = (struct record_list_node *)kmalloc(sizeof(struct record_list_node),
						 GFP_KERNEL);
	if (!rln)
		goto err_file_path;
	...
	if (filp->f_op) {
		errno = vfs_write(filp, (const char *)&write_record,
...
void record_illegal_root(const char *behavior, int cap)
{
	...
	write_record.uid = tmp_uid.val;
	write_record.euid = tmp_euid.val;
